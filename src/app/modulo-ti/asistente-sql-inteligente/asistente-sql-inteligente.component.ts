...
export class AsistenteSqlInteligenteComponent implements OnInit {
  ...
  highlightSQL(text: string): string {
    ...
    const keywords = /\b(SELECT|FROM|INNER\s+JOIN|LEFT\s+JOIN|RIGHT\s+JOIN|ON|WHERE|AND|OR|BETWEEN|ORDER\s+BY|GROUP\s+BY|INSERT\s+INTO|VALUES|UPDATE|SET|DELETE)\b/gi;
