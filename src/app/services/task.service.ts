...
import { resumenConsultaBoletosPrompt } from '../prompts/resumen-consulta-boletos.prompt';
import { SqlInteligenteService } from './sql-metadata.service';
...
export class TaskService {
  ...
  resumenOpenAI(textoDelReporte: string){
    ...
    return this.http.post("https://api.openai.com/v1/chat/completions", {
      model: "gpt-4o-mini",
      messages: [
        { role: "system", content: resumenConsultaBoletosPrompt
        },
        { role: "user", content: textoDelReporte }
      ]
    }, {
  ...
  AsistenteSqlInteligente(textoDeLaConsulta: string) {
    return this.sqlInteligenteService.generarPrompt().then((prompt) => {
      return this.http.post(
        "https://api.openai.com/v1/chat/completions",
        {
          ...
          messages: [
            ...
            { role: "user", content: textoDeLaConsulta },
          ],
        },
        {
          headers: {
